...
use ruma::{
    api::client::{
        ...
        keys::{
            ...
            upload_keys::unstable::{
                Request as UploadKeysRequestUnstable, Response as UploadKeysResponseUnstable,
            },
...
impl OlmMachine {
    ...
    async fn receive_keys_upload_response(
        &self,
        response: &UploadKeysResponseUnstable,
    ) -> OlmResult<()> {
        self.inner
            .store
    ...
    async fn keys_for_upload(&self, account: &Account) -> Option<UploadKeysRequestUnstable> {
        let (device_keys, one_time_keys, fallback_keys, one_time_cryptoids) =
            account.keys_for_upload();
            ...
            && one_time_cryptoids.is_empty()
        {
            ...
            Some(assign!(UploadKeysRequestUnstable::new(), {
    ...
    pub(crate) async fn preprocess_sync_changes(
...

        {
            ...
            account.update_cryptoid_counts(sync_changes.one_time_cryptoids_counts)
        }
    ...
    pub async fn get_cryptoid_for_room(
        &self,
        room: &str,
    ) -> Result<Option<Ed25519SecretKey>, CryptoStoreError> {
        let cache = self.inner.store.cache().await?;
        let account = cache.account().await?;
        Ok(account.get_cryptoid_for_room(room))
    }
    ...
    pub async fn claim_one_time_cryptoid_for_room(
        ...
        self.inner
            ...
            .with_transaction(|mut tr| async {
                let account = tr.account().await?;
                if let Err(_) = account.claim_one_time_cryptoid_for_room(room, key) {
                    return Err(OlmError::CryptoIDError);
                };
                Ok((tr, ()))
            })
            .await
    ...
    pub async fn create_cryptoid(&self) -> Result<Ed25519SecretKey, OlmError> {
        let mut crypto_id = Ed25519SecretKey::new();
        self.inner
            .store
            .with_transaction(|mut tr| async {
                let account = tr.account().await?;
                crypto_id = account.create_cryptoid();
                Ok((tr, ()))
            })
            .await?;
        Ok(crypto_id)
...
pub(crate) mod tests {
    ...
    fn keys_upload_response() -> UploadKeysResponseUnstable {
        let data = response_from_file(&test_json::KEYS_UPLOAD);
        UploadKeysResponseUnstable::try_from_http_response(data)
    ...
    pub(crate) async fn get_prepared_machine_test_helper(
        ...
        let request = machine
            ...
            .with_transaction(|mut tr| async {
                ...
                account.update_uploaded_cryptoid_count(0);
                account.generate_one_time_keys();
                account.generate_one_time_cryptoids();
                let request = machine
    ...
    async fn get_machine_after_query_test_helper() -> (OlmMachine, OneTimeKeys, OneTimeCryptoIDs) {
        let (machine, otk, otp) = get_prepared_machine_test_helper(user_id(), false).await;
    ...
    async fn get_machine_pair(
        ...
        let (bob, otk, otp) = get_prepared_machine_test_helper(bob, use_fallback_key).await;
    ...
    async fn get_machine_pair_with_session(
        ...
        let (alice, bob, mut one_time_keys, _) =
            get_machine_pair(alice, bob, use_fallback_key).await;
    ...
    async fn test_keys_query() {
        let (machine, _, _) = get_prepared_machine_test_helper(user_id(), false).await;
        let response = keys_query_response();
    ...
    async fn test_missing_sessions_calculation() {
        let (machine, _, _) = get_machine_after_query_test_helper().await;
    ...
    async fn test_decrypt_unencrypted_event() {
        let (bob, _, _) = get_prepared_machine_test_helper(user_id(), false).await;
        let room_id = room_id!("!test:example.org");
