...
//! Helpers to utilize the secret storage mechanism of the Matrix [spec].
//!
...
pub enum DecodeError {
    ...
    /// The secret storage key is using a unsupported secret encryption
    ...
    #[error("The secret storage key is using a unsupported secret encryption algorithm")]
...
///
/// The secret storage key can be initialized from a passphrase or from a Base58
/// encoded string.
///
/// To bootstrap a new [`SecretStorageKey`] use the [`SecretStorageKey::new()`]
/// or [`SecretStorageKey::new_from_passphrase()`] method.
///
/// After a new [`SecretStorageKey`] has been created, info about the key needs
/// to be uploaded to the homeserver as a global account data event. The event,
/// and event type, for this can be retrieved using the
...
pub struct AesHmacSha2EncryptedData {
    ...
    /// The message auhtentication code ensuring that the message was not
    /// forged.
    pub mac: [u8; MAC_SIZE],
}
...
impl SecretStorageKey {
    ...
    fn check_zero_message(&self) -> Result<(), DecodeError> {
        match &self.storage_key_info.algorithm {
            SecretEncryptionAlgorithm::SecretStorageV1AesHmacSha2 { iv, mac } => {
                if iv.as_bytes().len() != IV_SIZE {
                    Err(DecodeError::IvLength(IV_SIZE, iv.as_bytes().len()))
                } else {
                    ...
                    let ciphertext = key.encrypt_with_iv(Self::ZERO_MESSAGE.to_vec(), &iv_array);
    ...
    fn parse_base58_key(value: &str) -> Result<Box<[u8; 32]>, DecodeError> {
        ...
        if decoded_len != Self::DECODED_BASE58_KEY_LEN {
            ...
            let prefix_choice = prefix.ct_ne(&Self::PREFIX);
            let parity_choice = expected_parity.ct_ne(&parity);

            if prefix_choice.into() {
                Err(DecodeError::Prefix(Self::PREFIX, prefix))
            } else if parity_choice.into() {
    ...
    /// [`SecretStorageKey`] as a string. The caller needs to ensure to
    /// zeroize this string.
    ///
    /// [spec]: https://spec.matrix.org/v1.8/client-server-api/#key-representation
    ...
    /// Encrypt the given plaintext containing a secret storage secret with the
    /// given secret name.
    ///
    /// # Examples
    ///
    ...
    ///
    /// The [`SecretStorageKeyEventContent`] will contain information about this
    /// secret storage key which then can be used to check if the user has
    /// entered the correct key when trying to restore a [`SecretStorageKey`]
    /// using the [`SecretStorageKey::from_account_data()`] method.
