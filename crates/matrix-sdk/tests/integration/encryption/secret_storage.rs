...
async fn secret_store_missing_key_info() {
    ...
    let found_key_id = assert_matches!(
        ret,
        Err(SecretStorageError::MissingKeyInfo { key_id }) => key_id,
...
async fn secret_store_opening() {
    ...
    Mock::given(method("GET"))
        .and(path(format!(
            "_matrix/client/r0/user/@example:localhost/account_data/m.cross_signing.master"
        )))
        .and(header("authorization", "Bearer 1234"))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            "encrypted": {
...
async fn set_in_secret_store() {
...

    {
        ...
        let _guard = Mock::given(method("GET"))
            .and(path(format!("_matrix/client/r0/user/@example:localhost/account_data/foo")))
            .and(header("authorization", "Bearer 1234"))
            .respond_with(ResponseTemplate::new(404).set_body_json(json!({
                "errcode": "M_NOT_FOUND",
                "error": "Account data not found"
        ...
        secret_store
            .store_secret(&SecretName::from("foo"), "It's a secret to everybody")
            .await
    ...
    let secret = secret_store
        .get_secret(&SecretName::from("foo"))
        .await
        .expect("We should be able to retrieve a secret from the secret store")
        .unwrap();
